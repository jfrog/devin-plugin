...
async function main() {
  const manifestPath = path.join(repoRoot, ".devin-plugin", "plugin.json");
  const pluginManifest = await readJsonFile(manifestPath, "Plugin manifest (.devin-plugin/plugin.json)");
  if (!pluginManifest) {
    summarizeAndExit();
    return;
  }
