...
- **`<JFROG_PROJECT_KEY>` is always mandatory.** Resolve via the project
  ...
  `JF_PROJECT` env var (if set / non-empty, use `--project "$JF_PROJECT"`
  immediately — do NOT ask the user; pass the env var through, do not
  re-type it from other command output) → ASK the user. If none resolves,
  STOP and ask — NEVER guess, NEVER assume `default`, NEVER invent JFrog
  project keys.
