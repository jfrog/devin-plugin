...
- **Devin CLI:** run `/mcp` or `devin mcp list` for live connection status.
- **Devin Desktop (Local):** run `/mcp` for live connection status when
  available. If the config alone does not show what you need, tell the user to
  open **Open customizations** (MCP list) and report each server's status.
...
- **Devin Desktop (Local):** after the user reloads (see Restart), confirm `<name>`
  is connected, then **list that server's live tools** through the connected MCP.
- **Devin CLI:** after the user starts a new session (see Restart), confirm
  `<name>` via `/mcp` or `devin mcp list` if helpful, then **list that server's
  live tools** through the connected MCP.
...
each MCP tool call by default — tell the user to grant the prompt before
